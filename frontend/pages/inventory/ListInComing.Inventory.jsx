...
import { useGetAllInventoryIncomingQuery } from '../../redux/api/inventorySlice';
import Loading from '../../components/loading/Loading';
import IconBtn from '../../components/Shared/IconBtn';
import { useNavigate, Link } from 'react-router-dom';
import { useAcceptIncomingInventoryMutation } from '../../redux/api/inventorySlice';
...
const ListInComingInventory = () => {
  ...
  const { data: items, isLoading } = useGetAllInventoryIncomingQuery({
    page: page,
  });

  const [acceptIncomingLog] = useAcceptIncomingInventoryMutation();
  const [refuseIncomingLog] = useRefuseIncomingExportInventoryMutation();
  ...
  const handleAcceptLog = async (id) => {
    try {
      await acceptIncomingLog(id).unwrap();
  ...
  const handleRefuseLog = async (id) => {
    try {
      await refuseIncomingLog(id).unwrap();
    ...
    <div className="ml-[19rem]">
      <div className="w-[96%] flex flex-row justify-between">
        ...
        <div className="flex flex-row gap-4 mt-1">
          <Link to="/inventory/incoming/create">
            <IconBtn
              icon={
                <svg
          ...
          <Link to="/inventory/incoming/history">
            <IconBtn
              icon={
                <svg
                  xmlns="http://www.w3.org/2000/svg"
...
export default ListInComingInventory;
